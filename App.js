...
// ==================== LOGIN SCREEN ====================
function LoginScreen({ onSwitchToRegister, onSwitchToForgot, onLogin }) {
    ...
    <div className="auth-container">
      <div className="auth-card card">
        <div className="auth-header">
          ...
          <h2>Welcome Back</h2>
          <p className="auth-subtitle">Sign in to your SignetSure account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div className="auth-options">
            ...
            <button type="button" className="forgot-link" onClick={onSwitchToForgot}>
              Forgot password?
            </button>
          </div>
        ...
        <div className="social-login">
          <button className="social-btn google"><span style={{fontSize:'18px'}}>G</span> Google</button>
          <button className="social-btn facebook"><span style={{fontSize:'18px'}}>f</span> Facebook</button>
          <button className="social-btn apple"><span style={{fontSize:'18px'}}>🍎</span> Apple</button>
        </div>
...
// ==================== REGISTER SCREEN ====================
function RegisterScreen({ onSwitchToLogin, onRegister }) {
  ...
  const getPasswordStrengthColor = () => {
    const colors = ['#E24B4A', '#EF9F27', '#378ADD', '#0F6E56'];
    return colors[passwordStrength - 1] || '#E24B4A';
  };
            ...
            {formData.password && (
              <div className="password-strength">
                <div className="strength-bar">
                  <div className="strength-fill" style={{ width: `${(passwordStrength / 4) * 100}%`, background: getPasswordStrengthColor() }} />
                </div>
      ...
      <div className="auth-container">
        <div className="auth-card card">
          <div className="auth-header">
            ...
            <p className="auth-subtitle">We've sent a password reset link to</p>
            <p className="auth-email-display">{email}</p>
          </div>
          <div className="auth-success">✅ Password reset link sent!</div>
          <div className="reset-instructions">
            <p>📌 Click the link in the email to reset your password.</p>
            <p>⏰ The link will expire in 1 hour.</p>
            <p>📬 If you don't see the email, check your spam folder.</p>
          </div>
          <button className="auth-submit-btn back-to-login" onClick={onSwitchToLogin}>
            ← Back to Sign In
          </button>
        </div>
      </div>
  ...
  return (
    <div className="auth-container">
      <div className="auth-card card">
        ...
        <div className="auth-footer">
          <button className="auth-switch-btn back-link" onClick={onSwitchToLogin}>
            ← Back to Sign In
          </button>
        </div>
      </div>
    ...
    <>
      ...
      <div className="card">
        ...
        <div className="sched-row" style={{border:'none',padding:'4px 0'}}>
          ...
          <button className="btn sm primary">View Schedule</button>
        </div>
      </div>
  ...
  return (
    <div className="vitals-detail-view">
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <button className={`btn ${selectedVital === 'all' ? 'primary' : ''}`} onClick={() => setSelectedVital('all')} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer' }}>All Vitals</button>
        <button className={`btn ${selectedVital === 'bp' ? 'primary' : ''}`} onClick={() => setSelectedVital('bp')} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer' }}>❤️ BP</button>
        <button className={`btn ${selectedVital === 'oxygen' ? 'primary' : ''}`} onClick={() => setSelectedVital('oxygen')} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer' }}>🫁 Oxygen</button>
        <button className={`btn ${selectedVital === 'temp' ? 'primary' : ''}`} onClick={() => setSelectedVital('temp')} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer' }}>🌡️ Temp</button>
      </div>

      <div className="stats-grid" style={{ marginBottom: '16px' }}>
        <div className="stat-card"><div className="stat-label">Avg BP</div><div className="stat-value ok">{avgSys}<span className="stat-unit">/{avgDia}</span></div><div className="stat-trend" style={{ color: 'var(--color-text-secondary)' }}>mmHg</div></div>
        <div className="stat-card"><div className="stat-label">Avg Oxygen</div><div className="stat-value ok">{avgOxygen}<span className="stat-unit">%</span></div><div className="stat-trend" style={{ color: 'var(--color-text-secondary)' }}>SpO₂</div></div>
        <div className="stat-card"><div className="stat-label">Avg Temp</div><div className="stat-value ok">{avgTemp}<span className="stat-unit">°F</span></div><div className="stat-trend" style={{ color: 'var(--color-text-secondary)' }}>°F</div></div>
        <div className="stat-card"><div className="stat-label">Avg Heart Rate</div><div className="stat-value ok">{avgHR}<span className="stat-unit"> bpm</span></div><div className="stat-trend" style={{ color: 'var(--color-text-secondary)' }}>BPM</div></div>
      </div>

      <div className="card" style={{ marginBottom: '12px' }}>
        <div className="card-header">
          <span className="card-title"><span style={{ color: 'var(--teal)' }}>📈</span> {selectedVital === 'all' ? 'Heart Rate Trend' : selectedVital === 'bp' ? 'Blood Pressure (Systolic)' : selectedVital === 'oxygen' ? 'Oxygen Saturation' : 'Temperature'}</span>
          <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>{selectedVital === 'all' ? 'bpm' : selectedVital === 'bp' ? 'mmHg' : selectedVital === 'oxygen' ? '%' : '°F'}</span>
        </div>
        <div className="chart-container" style={{ position: 'relative', width: '100%', overflowX: 'auto' }}>
          <svg width={chartData.length * 45 + 20} height="200" viewBox={`0 0 ${chartData.length * 45 + 20} 200`} style={{ display: 'block', margin: '0 auto' }}>
            {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
              const y = 20 + (180 - 40) * (1 - ratio);
              const value = minValue + range * ratio;
              return (
                <g key={ratio}>
                  <line x1="10" y1={y} x2={chartData.length * 45 + 10} y2={y} stroke="#E5E2DC" strokeWidth="0.5" strokeDasharray="4" />
                  <text x="8" y={y + 4} fontSize="9" fill="#6B6A67" fontFamily="var(--font)" textAnchor="end">{Math.round(value)}</text>
                </g>
              );
            })}
            <defs>
            ...
            <path d={`${generatePath()} L ${chartData.length * 45 + 10} ${180} L 10 ${180} Z`} fill="url(#areaGradient)" />
            <path d={generatePath()} stroke="#0F6E56" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
            {chartData.map((d, i) => {
              const pos = getPointPosition(i);
              const color = getStatusColor(selectedVital === 'all' ? 'heart' : selectedVital, d.value);
              const isHovered = hoveredPoint === i;
              return (
                <g key={i} onMouseEnter={() => setHoveredPoint(i)} onMouseLeave={() => setHoveredPoint(null)} style={{ cursor: 'pointer' }}>
                  <circle cx={pos.x} cy={pos.y} r={isHovered ? 7 : 4} fill={isHovered ? '#0F6E56' : color} stroke="white" strokeWidth="2" opacity={isHovered ? 1 : 0.8} />
                  {isHovered && (
                    <>
                      <rect x={pos.x - 30} y={pos.y - 38} width="60" height="28" rx="4" fill="white" stroke="#E5E2DC" strokeWidth="0.5" />
                      <text x={pos.x} y={pos.y - 20} fontSize="9" fill="#1C1B1A" fontFamily="var(--font)" textAnchor="middle" fontWeight="500">{d.value}{d.unit}</text>
                    </>
                  )}
                </g>
              );
            ...
            {chartData.map((d, i) => {
              const pos = getPointPosition(i);
              return (
                <text key={i} x={pos.x} y="195" fontSize="8" fill="#6B6A67" fontFamily="var(--font)" textAnchor="middle">{d.label}</text>
              );
            })}
          </svg>
        </div>
        <div style={{ display: 'flex', gap: '16px', marginTop: '8px', paddingTop: '8px', borderTop: '0.5px solid var(--color-border-tertiary)', fontSize: '10px', color: 'var(--color-text-secondary)', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '4px' }}>
        {chartData.map((d, i) => {
          const color = getStatusColor(selectedVital === 'all' ? 'heart' : selectedVital, d.value);
          return (
            <div key={i} style={{ background: hoveredPoint === i ? 'var(--teal-light)' : 'var(--color-background-secondary)', borderRadius: '4px', padding: '4px 6px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.15s', border: hoveredPoint === i ? '0.5px solid var(--teal)' : 'none' }}
        ...
        <div id="med-schedule">
          {medications.map(med => (
            <div className="sched-row" key={med.id}>
              ...
              <div className={`sched-pill-icon ${med.am ? 'am' : 'pm'}`}><span>💊</span></div>
              <div style={{flex:1}}>
...
function CaregiverScreen() {
  ...
  const toggleAlert = (index) => {
    setAlertSettings(prev => prev.map((a, i) => i === index ? { ...a, checked: !a.checked } : a));
  };
    ...
    <div className="card" style={{padding:0,overflow:'hidden'}}>
      ...
      <div style={{padding:'12px'}}>
            ...
            <div key={msg.id} className={`chat-msg ${msg.sender}`}>
              ...
              {msg.sender === 'bot' && msg.id === 1 && (
                <div className="feeling-btns">
                  <button className="feel-btn" onClick={() => handleSend('Yes, I ate')}>Yes, I ate</button>
                  <button className="feel-btn" onClick={() => handleSend('Forgot to eat')}>Forgot to eat</button>
                </div>
              )}
              {msg.sender === 'bot' && msg.id === 2 && (
                <div className="feeling-btns">
                  <button className="feel-btn" onClick={() => handleSend('Drinking plenty')}>Drinking plenty</button>
                  <button className="feel-btn" onClick={() => handleSend('Not enough water today')}>Need to drink more</button>
                  <button className="feel-btn" onClick={() => handleSend('I am experiencing side effects: dizziness')}>Side effects</button>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="chat-input-row">
          ...
          <button className="chat-send" onClick={() => handleSend(inputValue)}><span>📤</span></button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      <div className="card">
        ...
        <button className="btn primary" style={{width:'100%',justifyContent:'center'}}>+ Add Payment Method</button>
      </div>
